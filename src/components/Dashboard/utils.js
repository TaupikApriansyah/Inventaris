export const getDashboardStats = (dataItems) => {
    const kibItems = dataItems.filter(item => item.jenis === 'KIB' && item.nilaiNumerik > 0);

    const totalAset = kibItems.reduce((sum, item) => sum + item.jumlah, 0);
    const totalValuasi = kibItems.reduce((sum, item) => sum + item.nilaiNumerik, 0);
    const asetRusakBerat = kibItems.filter(item => item.kondisi === 'Rusak Berat').reduce((sum, item) => sum + item.jumlah, 0);
